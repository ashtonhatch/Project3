...
Map::Map()
{
    //cout  << endl << " I AM RESETTING THE MAP!!!!" << endl  << endl << endl << endl << endl << endl;
    resetMap();
}
...
{    
    for ( int i = 0; i < max_rooms_; i++ ) {
                ...
                for ( int ci = 0; ci < 2; ci++ ) {
                    int valToPass = num_rows_;
                    if (ci == 1) {
                        valToPass = num_cols_;
                    }
                    room_positions_[i][ci] = generateRandomInt(valToPass);
                    
                }
                if((room_positions_[i][0] == 0 && room_positions_[i][1] == 0 ) || (room_positions_[i][0] == num_cols_ % 2 && room_positions_[i][1] == 11 )){    // dungeon exit always in the middle
                    i--;
                }
                else {addRoom(room_positions_[i][0], room_positions_[i][1]);}

        }
}


int Map::generateRandomInt(int valToPass) {
    int randnum = (rand() % valToPass);   // change map size by passing either num_rows_ or num_cols_ in map.h
